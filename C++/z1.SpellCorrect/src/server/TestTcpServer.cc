...
{
public:
    Task(const string & msg, const wd::TcpConnectionPtr & conn) : _msg(msg) , _conn(conn) {}
    //运行在线程池的某一个子线程中
    void process()
    {
        //decode
...
int main(void)
{
    ...
    wd::TcpServer server("172.19.210.35", 1234);
    server.setConnectionCallback(onConnection);
    server.setMessageCallback(onMessage);
    server.setCloseCallback(onClose);
    server.start();
    //EchoServer e0(4,10,"172.19.210.35",1234);
    //e0.Start();
    return 0;
}
