        ...
        if(warrior1 && warrior2)
        {
            ...
            if(_id%2)//奇数方红先攻
            {
                ...
                {//000:40 red iceman 1 killed blue lion 12 in city 2 remaining 20 elements
                    while(1)
                    {
                        if(warrior1->_weaponA.size()!=0)
                        {
                            warrior2->get_harmed( warrior1->use_weapon());
                            if(warrior2->get_elements()<0) { if(warrior1->get_elements()<0) { print_time(); printf("both red %s %d and blue %s %d died in city %d\n",warriors[warrior1->get_type()],warrior1->get_id(), warriors[warrior2->get_type()],warrior2->get_id(),_id); return; }
                                print_time(); printf("red %s %d killed blue %s %d int city %d remaining %d elements\n", warriors[warrior1->get_type()],warrior1->get_id(),warriors[warrior2->get_type()],warrior2->get_id(), _id,warrior1->get_elements()); return; }
                        }
                        if(warrior2->_weaponA.size()!=0)
                        {
                            warrior1->get_harmed( warrior2->use_weapon());
                            if(warrior1->get_elements()<0) { if(warrior2->get_elements()<0) { print_time(); printf("both red %s %d and blue %s %d died in city %d\n",warriors[warrior1->get_type()],warrior1->get_id(), warriors[warrior2->get_type()],warrior2->get_id(),_id); return; }
                                print_time(); printf("blue %s %d killed red %s %d int city %d remaining %d elements\n", warriors[warrior2->get_type()],warrior2->get_id(),warriors[warrior1->get_type()],warrior1->get_id(), _id,warrior2->get_elements()); return; }
                        }
                    ...
                    while(1)
                    {
                        if(warrior2->_weaponA.size()!=0)
                        {
                            ...
                            if(warrior1->get_elements()<0)
                            {
                                ...
                                print_time();
                                printf("blue %s %d killed red %s %d int city %d remaining %d elements\n",
                                       warriors[warrior2->get_type()],warrior2->get_id(),warriors[warrior1->get_type()],warrior1->get_id(), _id,warrior2->get_elements());
                                return;
                            }
                        }
                        if(warrior1->_weaponA.size()!=0)
                        {
                            ...
                            if(warrior2->get_elements()<0)
                            {
                                ...
                                print_time();
                                printf("red %s %d killed blue %s %d int city %d remaining %d elements\n", warriors[warrior1->get_type()],warrior1->get_id(),warriors[warrior2->get_type()],warrior2->get_id(), _id,warrior1->get_elements());
                                return;
                            }
                        }
                        if( !warrior1->is_changed && !warrior2->is_changed)
