...
int main()
{
    ...
    int command_result=999;
        ...
        //执行命令并获得结果
            //command_result=server_recv_file (client_fd);
            //server_ls(client_fd);
            //server_cd(client_fd);
            //command_result=server_send_file(client_fd);
            //command_result=server_pwd(client_fd);
           // command_result=server_mkdir(client_fd);
            command_result=server_remove(client_fd);
        //发送执行结果
        printf("command is %s\n",buf);
